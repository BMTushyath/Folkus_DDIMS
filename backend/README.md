...
It is composed of a React frontend and a Node.js backend, organized into `frontend` and `backend` directories.
...
**b. Run the Frontend:**
The frontend can be served directly from the `index.html` file using any local development server. A simple way to run it is using the `serve` package from the project's root directory (the one containing `frontend` and `backend`).

```bash
# From the project root, this command serves the contents of the 'frontend' directory
npx serve frontend
```
...
- **Backend:** The Express server in `backend` receives these requests, manages user data (in memory for this demo), and makes secure calls to the Google Gemini API for the AI Security Audit and translation features. Your API key is never exposed to the browser.